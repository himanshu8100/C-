...
ChangeContrast(double factor)
{
  ...
  // and negative factors generate inverted images.
      ...
      for (int i = 0; i < Width(); i++)
         for (int j = 0; j < Height(); j++)
            total_luminance += Pixels(i)[j].Luminance();

      total_luminance /= Height()*Width();
      ...
      for (int i = 0; i < Width(); i++){
         ...
         {
            R2Pixel& pixel = Pixel(i, j);
            pixel[0] = (1-factor)*total_luminance + pixel[0]*factor;
            pixel[1] = (1-factor)*total_luminance + pixel[1]*factor;
            pixel[2] = (1-factor)*total_luminance + pixel[2]*factor;
            pixel.Clamp();
         }
      }
  // FILL IN IMPLEMENTATION HERE (REMOVE PRINT STATEMENT WHEN DONE)
...
Blur(double sigma)
{
    ...
    int size_gaussian= ceil(6*sigma+1);
    double kernel[size_gaussian][size_gaussian];
    //sum for normalization
    double r,sum = 0.0;
    int radius= (size_gaussian-1)/2;

    //Initializing Gaussian
    for (int i = 0; i < size_gaussian; i++){
         for (int j = i; j < size_gaussian; j++){
            r = (i-radius) * (i-radius)+ (j-radius) * (j-radius);
            kernel[i][j]=(exp(-(r) / (2*sigma*sigma)));
            if (i!=j){
    ...
    //Normalizing
    for (int x = 0; x < size_gaussian; ++x)
        for (int y = 0; y < size_gaussian; ++y)
            kernel[x][y] /= sum;

	int x_pos, y_pos, sum_r, sum_b, sum_g;
      ...
      for (int r = 0; r < Width(); r++)
      {
         for (int c = 0; c < Height(); c++)
         {
            ...
            for (int i = 0; i < size_gaussian; i++)
            {
               ...
               for (int j = 0; j < size_gaussian; j++)
               {
					x_pos = r + (j - radius);
					if (x_pos < 0 || x_pos >= (Width()))
                     break;

					sum_r += Pixels(x_pos)[y_pos].Red()*kernel[i][j];
					sum_g += Pixels(x_pos)[y_pos].Blue()*kernel[i][j];
					sum_b += Pixels(x_pos)[y_pos].Green()*kernel[i][j];

			    }
            }
			tempimage->Pixels(r)[c].SetRed(sum_r);
            tempimage->Pixels(r)[c].SetBlue(sum_b);
            tempimage->Pixels(r)[c].SetGreen(sum_g);
		}
	 }
...
Sharpen()
{
	// Blur an image with a Gaussian filter with a given sigma.
    int size_linear= 3;
    double kernel[size_linear][size_linear]= {{-1/9., -1/9., -1/9.},{-1/9.,9,-1/9.},{-1/9.,-1/9.,-1/9}};
    int radius= (size_linear-1)/2;
...
EdgeDetect(void)
{
    ...
    int size_linear= 5;
    double kernel[size_linear][size_linear]= {{0, 0, -1, 0, 0},{0, -1, -2, -1, 0},{-1,-2,16,-2,-1},{0, -1, -2, -1, 0},{0, 0, -1, 0, 0}};//Marr-Hildreth
    int radius= (size_linear-1)/2;
